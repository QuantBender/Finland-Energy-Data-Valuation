...
def read_ercot_da_ccp(fname, month):
    ...
    if len(df1) > 0:
        ...
        try:
            crossborder = df1['CROSSBORDER '].astype('float').values  # why is there an extra space in the key
        except KeyError:
            crossborder = df1['CROSSBORDER'].astype('float').values
        finally:
            crossborder = crossborder[~np.isnan(crossborder)]
