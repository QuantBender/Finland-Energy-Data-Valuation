...
class ValuationDMS(DataManagementSystem):
    ...
    def get_ercot_spp_data(self, id_key):
        """Retrieves DAM-SPP data for FI."""
        logging.info('DMS: Loading FI DA-SPP')
        try:
    ...
    def get_ercot_ccp_data(self, id_key):
        """Retrieves DAM-CCP data for FI."""
        logging.info('DMS: Loading FI DA-CCP')
        try:
            # attempt to access data if it is already loaded
            REGUP = self.get_data(id_key + self.delimiter + 'REGUP')
            REGDN = self.get_data(id_key + self.delimiter + 'REGDN')
            CROSSBORDER = self.get_data(id_key + self.delimiter + 'CROSSBORDER')
        except KeyError:
            # load the data and add it to the DMS

            # deconstruct id_key to obtain args for read function
            REGDN, REGUP, CROSSBORDER = read_ercot_da_ccp(*id_key.split(self.delimiter)[:2])

            self.add_data(REGUP, id_key + self.delimiter + 'REGUP')
            self.add_data(REGDN, id_key + self.delimiter + 'REGDN')
            self.add_data(CROSSBORDER, id_key + self.delimiter + 'CROSSBORDER')
        finally:
            return REGDN, REGUP, CROSSBORDER

    def get_ercot_data(self, year, month, settlement_point):
        # construct file name paths
        path = os.path.join(self.home_path, 'FI')  # path to data_bank root
        ...
        rd, ru, cb = self.get_ercot_ccp_data(ccp_id)

        return spp_da, rd, ru, cb
...
if __name__ == '__main__':
    ...
    # # FI - data doesn't exist
    # year = 2010
    # month = 1
    # settlement_point = 'HB_HOUSTON'

    # spp_da, rd, ru, cb = dms.get_ercot_data(year, month, settlement_point)
