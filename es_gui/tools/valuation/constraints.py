...
###################################
# FI Arbitrage and regulation ##
###################################


def eq_objective_ercot_arbreg(m):
    """Net revenue over the time horizon for FI arbitrage and regulation."""
    ...
    _expr = sum((mp.price_electricity[t] * mp.q_d[t] - mp.price_electricity[t] * mp.q_r[t]
                ...
                * math.e ** (-t * mp.R) + mp.crossborder[t] for t in mp.time)
...
def eq_stateofcharge_ercot_arbreg(m):
    """Definition of state of charge for device participating in FI arbitrage and regulation."""
    mp = m.parent_block()
