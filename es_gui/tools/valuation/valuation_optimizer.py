...
class ValuationOptimizer(optimizer.Optimizer):
    ...
    def __init__(self, price_electricity=None,
                 price_reg_up=None, price_reg_down=None, crossborder=None,
    ...
    def crossborder(self):
        """The income from crossborder electricity exchange [€]."""
        return self._crossborder

    @crossborder.setter
    def crossborder(self, value):
        self._crossborder = value
    ...
    def _set_model_param(self):
        ...
        if self.market_type in {'caiso_pfp'}: 
            # TODO: Figure out SPP for this?
    ...
    def instantiate_model(self):
        ...
        m.crossborder = self.crossborder
    ...
    def _process_results(self):
        ...
        #######################################################################################################################
            ...
            #                               - m.q_rd[t].value * m.fraction_reg_down[t])
            #                               for t in m.time]))
            # rev_crossborder = np.cumsum(np.array([m.crossborder[t] for t in m.time]))
            ...
            revenue = rev_arb + rev_reg + rev_crossborder

            run_results['rev_arb'] = rev_arb
            run_results['rev_reg'] = rev_reg
            run_results['crossborder'] = rev_crossborder
            run_results['revenue'] = revenue
        elif self.market_type == 'fi_arbreg':
            # rev_arb = np.cumsum(np.array([m.price_electricity[t]*(m.q_d[t].value - m.q_r[t].value
            #                                                       + m.q_ru[t].value*m.fraction_reg_up[t]
            #                                                       - m.q_rd[t].value*m.fraction_reg_down[t])
            #                               for t in m.time]))
            ...
            # rev_reg = np.cumsum(np.array([m.price_reg_up[t] * m.q_ru[t].value + m.price_reg_down[t] * m.q_rd[t].value
            #                               + m.price_electricity[t]*(m.q_ru[t].value * m.fraction_reg_up[t]
            #                               - m.q_rd[t].value * m.fraction_reg_down[t] + m.crossborder[t].value)
            #                               for t in m.time]))
